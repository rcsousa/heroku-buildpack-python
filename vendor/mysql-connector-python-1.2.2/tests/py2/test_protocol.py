...
class MySQLProtocolTests(tests.MySQLConnectorTests):
    ...
    def test_make_auth_ssl(self):
        ...
        cases = [
            ({},
             '\x00\x00\x00\x00\x00\x00\x00\x40\x21\x00\x00\x00\x00\x00'
             '\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00'
             '\x00\x00\x00\x00'),
            ({'charset': 8},
             '\x00\x00\x00\x00\x00\x00\x00\x40\x08\x00\x00\x00\x00\x00'
             '\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00'
             '\x00\x00\x00\x00'),
            ({'client_flags': 240141},
             '\x0d\xaa\x03\x00\x00\x00\x00\x40\x21\x00\x00\x00\x00\x00'
             '\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00'
             '\x00\x00\x00\x00'),
            ({'charset': 8, 'client_flags': 240141,
              'max_allowed_packet': 2147483648},
                '\x0d\xaa\x03\x00\x00\x00\x00\x80\x08\x00\x00\x00\x00\x00'
                '\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00'
                '\x00\x00\x00\x00'),
        ]
    ...
    def test_make_command(self):
        ...
        exp = '\x01\x68\x61\x6d'
        res = self._protocol.make_command(1, 'ham')
        self.assertEqual(exp, res)
        res = self._protocol.make_command(1, argument='ham')
        self.assertEqual(exp, res)
    ...
    def test_make_change_user(self):
    ...
    def test_parse_handshake(self):
        ...
        exp = {
            ...
            'capabilities': 41516,
            'server_status': 2,
            'auth_data': 'h4i6oP!OLng9&PD@WrYH',
            'auth_plugin': 'mysql_native_password',
        }
    ...
    def test_parse_binary_prepare_ok(self):
        ...
        cases = [
            # SELECT CONCAT(?, ?) AS c1
            ('\x0c\x00\x00\x01'
             '\x00\x01\x00\x00\x00\x01\x00\x02\x00\x00\x00\x00',
             {'num_params': 2,
              'statement_id': 1,
              'warning_count': 0,
              'num_columns': 1
    ...
    def test__parse_binary_integer(self):
        ...
        for exp, fmt, field_type, flag in cases:
            ...
            field_info[7] = flag
            data = struct.pack(fmt, exp) + '\x00\x00'
            res = self._protocol._parse_binary_integer(data, field_info)
            self.assertEqual(('\x00\x00', exp), res,
    ...
    def test__parse_binary_float(self):
        ...
        cases = [
            (-3.14159, '\x6e\x86\x1b\xf0\xf9\x21\x09\xc0', FieldType.DOUBLE),
            (3.14159, '\x6e\x86\x1b\xf0\xf9\x21\x09\x40', FieldType.DOUBLE),
            (-3.14, '\xc3\xf5\x48\xc0', FieldType.FLOAT),
            (3.14, '\xc3\xf5\x48\x40', FieldType.FLOAT),
        ]
        field_info = [None] * 8
        field_info[0] = 'c1'
        for exp, data, field_type in cases:
            field_info[1] = field_type
            res = self._protocol._parse_binary_float(data + '\x00\x00',
                                                     field_info)
            self.assertEqual('\x00\x00', res[0],
    ...
    def test__parse_binary_timestamp(self):
        ...
        cases = [
            (datetime.date(1977, 6, 14), '\x04\xb9\x07\x06\x0e'),
            (datetime.datetime(1977, 6, 14, 21, 33, 14),
             '\x07\xb9\x07\x06\x0e\x15\x21\x0e'),
            (datetime.datetime(1977, 6, 14, 21, 33, 14, 345),
             '\x0b\xb9\x07\x06\x0e\x15\x21\x0e\x59\x01\x00\x00')
        ]
        for exp, data in cases:
            res = self._protocol._parse_binary_timestamp(data + '\x00\x00',
                                                         None)
            self.assertEqual(('\x00\x00', exp), res,
    ...
    def test__parse_binary_time(self):
        ...
        cases = [
            (datetime.timedelta(0, 44130),
             '\x08\x00\x00\x00\x00\x00\x0c\x0f\x1e'),
            (datetime.timedelta(14, 15330),
             '\x08\x00\x0e\x00\x00\x00\x04\x0f\x1e'),
            (datetime.timedelta(-14, 15330),
             '\x08\x01\x0e\x00\x00\x00\x04\x0f\x1e'),
            (datetime.timedelta(10, 58530, 230000),
             '\x0c\x00\x0a\x00\x00\x00\x10\x0f\x1e\x70\x82\x03\x00'),
        ]
        for exp, data in cases:
            res = self._protocol._parse_binary_time(data + '\x00\x00', None)
            self.assertEqual(('\x00\x00', exp), res,
                             "Failed parsing time '{0}'".format(exp))

    def test__parse_binary_values(self):
        """Parse values from a binary result packet"""
        # Test is using following statement.
        # stmt = ("SELECT 'abc' AS aStr,"
        #        "3.14 AS aFloat,"
        #        "-3.14159 AS aDouble, "
        ...
        packet = ('\x00\x01\x03\x61\x62\x63\x04\x33\x2e\x31\x34\x08'
                  '\x2d\x33\x2e\x31\x34\x31\x35\x39\x04\xd3\x07'
                  '\x01\x1f\x07\xb9\x07\x06\x0e\x15\x21\x0e\x0c'
                  '\x00\x0a\x00\x00\x00\x10\x0f\x1e\x70\x82\x03\x00')
    ...
    def test__prepare_binary_timestamp(self):
        ...
        cases = [
            (datetime.date(1977, 6, 14), ('\x04\xb9\x07\x06\x0e', 10)),
            (datetime.datetime(1977, 6, 14),
             ('\x07\xb9\x07\x06\x0e\x00\x00\x00', 12)),
            (datetime.datetime(1977, 6, 14, 21, 33, 14),
             ('\x07\xb9\x07\x06\x0e\x15\x21\x0e', 12)),
            (datetime.datetime(1977, 6, 14, 21, 33, 14, 345),
             ('\x0b\xb9\x07\x06\x0e\x15\x21\x0e\x59\x01\x00\x00', 12)),
        ]
        for data, exp in cases:
            self.assertEqual(exp,
    ...
    def test__prepare_binary_time(self):
        ...
        cases = [
            (datetime.timedelta(hours=123, minutes=45, seconds=16),
             ('\x08\x00\x05\x00\x00\x00\x03\x2d\x10', 11)),
            (datetime.timedelta(hours=-123, minutes=45, seconds=16),
             ('\x08\x01\x06\x00\x00\x00\x15\x2d\x10', 11)),
            (datetime.timedelta(hours=123, minutes=45, seconds=16,
                                microseconds=345),
             ('\x0c\x00\x05\x00\x00\x00\x03\x2d\x10\x59\x01\x00\x00', 11)),
            (datetime.timedelta(days=123, minutes=45, seconds=16),
             ('\x08\x00\x7b\x00\x00\x00\x00\x2d\x10', 11)),
            (datetime.time(14, 53, 36),
             ('\x08\x00\x00\x00\x00\x00\x0e\x35\x24', 11)),
            (datetime.time(14, 53, 36, 345),
             ('\x0c\x00\x00\x00\x00\x00\x0e\x35\x24\x59\x01\x00\x00', 11))
        ]
        for data, exp in cases:
            self.assertEqual(exp,
                             self._protocol._prepare_binary_time(data),
                             "Failed preparing value '%s'" % data)
